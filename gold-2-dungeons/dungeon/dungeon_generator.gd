...
func generate_hallway_network() -> void:
	...
	for position in room_centers:
		var pos_2d: Vector2 = Vector2(position.x, position.z)
...
func create_hallways(hallway_graph: AStar2D) -> void:
	...
	for point in hallway_graph.get_point_ids():
		for connection in hallway_graph.get_point_connections(point):
			if connection > point:  # Process each connection only once
				...
				var from_door: Vector3 = find_closest_tile(from_room, room_centers[connection])
				var to_door: Vector3 = find_closest_tile(to_room, room_centers[point])
	...
	for endpoints in hallway_endpoints:
		...
		var from_pos: Vector2i = Vector2i(endpoints[0].x, endpoints[0].z)
		var to_pos: Vector2i = Vector2i(endpoints[1].x, endpoints[1].z)
		...
		# Place hallway tiles
		for point in path:
			var tile_pos: Vector3i = Vector3i(point.x, 0, point.y)
...
func find_closest_tile(room_tiles: PackedVector3Array, target: Vector3) -> Vector3:
	var closest_tile: Vector3 = room_tiles[0]
